...
//
template <class T, unsigned int VImageDimension >
typename ITKAffineTransform<T,VImageDimension>::Type::Pointer VectorImageUtils< T, VImageDimension >
::readAffineTransformITK(const std::string& filename) {

#if ((ITK_VERSION_MAJOR > 3) && (ITK_VERSION_MINOR >= 4))
  typedef itk::TransformFileReaderTemplate<T> XFRMReaderType;
#else
  typedef itk::TransformFileReader XFRMReaderType;
#endif
